import { useLayoutEffect, useRef, useCallback, ReactNode, Key } from 'react';
...
const ScrollStack = ({
  ...
  const setupLenis = useCallback(() => {
    if (useWindowScroll) {
      ...
      // Use native scroll instead of Lenis to allow proper scroll propagation and prevent trapping wheel events at boundaries
      ...
      (scroller as any)._handleWheel = handleWheel;
    ...
    return () => {
      ...
      if (lenisRef.current) {
        ...
        if (scrollerElement) {
          ...
          if ((scrollerElement as any)._handleWheel) {
            scrollerElement.removeEventListener('wheel', (scrollerElement as any)._handleWheel);
          }
        }
      }
      stackCompletedRef.current = false;
